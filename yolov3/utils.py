...
def isInBox(pointX, pointY, x1, x2, y1, y2):
    if pointX is None or pointY is None:
        return False
    if (pointX > x1 and pointX < x2 and pointY > y1 and pointY < y2):
        return True
    return False
...
def draw_bbox(image, bboxes, refPt, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
